...
abstract class AbstractFirDiagnosticsTestSpec : AbstractFirOldFrontendDiagnosticsTest() {
    ...
    override fun analyzeAndCheck(testDataFile: File, files: List<TestFile>) {
        ...
        val testMetaInfoPath = if (!testFilePath.contains(".fir.kt")) testFilePath else testFilePath.replace(".fir.kt", ".kt")
        specTest = CommonParser.parseSpecTest(testMetaInfoPath, files.associate { Pair(it.fileName, it.clearText) }).first
